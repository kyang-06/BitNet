...
# pydevd_pycharm.settrace('10.238.129.250', port=12312, stdout_to_server=True, stderr_to_server=True, suspend=False)
...
def convert_ts_checkpoint(
    *,
    ...
    def quant_weight_int8(weight):
        s = 1.0 / weight.abs().reshape(weight.shape[0], weight.shape[1] // group_size, group_size).mean(dim=-1, keepdims=True).clamp_(min=1e-5)
    ...
    for key, value in merged_result.items():
        if 'wqkv' in key:
            ...
            zero = torch.zeros_like(wa_scale)
            wqkv_weight = torch.cat([wq_weight, wk_weight, wv_weight], dim=0)
            wqkv_scale = torch.cat([wa_scale, wb_scale, wc_scale], dim=0)
            ...
            zero = torch.zeros_like(w1_scale)
            w13_weight = torch.cat([w1_weight, w3_weight], dim=0)
            w13_scale = torch.cat([w1_scale, w3_scale], dim=0)
    ...
    print(f"Saving checkpoint to {output_dir}/group_size_128/model_state_int2.pt")
    torch.save(int2_result, f"{output_dir}/group_size_128/model_state_int2.pt")

    print(f"Saving checkpoint to {output_dir}/group_size_128/model_state_fp16.pt")
    torch.save(fp16_result, f"{output_dir}/group_size_128/model_state_fp16.pt")
