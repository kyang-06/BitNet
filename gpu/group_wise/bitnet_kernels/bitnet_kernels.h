...
__global__ void __launch_bounds__(128) ladder_int8xint2_kernel(int8_t* __restrict__ A, int8_t* __restrict__ B, __nv_bfloat16* __restrict__ dtype_transform, __nv_bfloat16* __restrict__ s, __nv_bfloat16* __restrict__ ws) {
  ...
  __nv_bfloat16 red_buf0[1];
  __nv_bfloat16 ws_local[1];
  __nv_bfloat16 fp_thread_C_local[1];
  fp_thread_C_local[0] = 0.0f;
  #pragma unroll
  for (int k_0 = 0; k_0 < K/(K_per_loop * K_block_size); ++k_0) {
    *(int4*)(A_local + 0) = *(int4*)(A + ((k_0 * K_per_loop * K_block_size) + (((int)threadIdx.x) * K_per_loop)));
    ...
    int ws_linear_idx = (((int)blockIdx.x) * N_block_size + ((int)threadIdx.y)) * num_group + k_0;
    ws_local[0] = ws[ws_linear_idx];
    fp_thread_C_local[0] += (__nv_bfloat16)(in_thread_C_local[0])*(ws_local[0]);
// **************DEBUG USAGE************
//    int A_idx = (k_0 * K_per_loop * K_block_size) + (((int)threadIdx.x) * K_per_loop);
//    if ((ws_linear_idx / num_group) == 0 || (ws_linear_idx / num_group) == 960 || (ws_linear_idx / num_group) == 1920 || (ws_linear_idx / num_group) == 2880) {
//        __nv_bfloat16 val = ws[ws_linear_idx];
//        unsigned short hex_val = *(unsigned short*)&val;
//        printf("DEBUG: <BlockX=%d, ThreadX=%d, ThreadY=%d>, A_idx_st=%d -> WS_Idx=(%d, %d), WS_value=%f, WS_raw_value=0x%04x\n",
//                       blockIdx.x, threadIdx.x, threadIdx.y, A_idx, ws_linear_idx / num_group, ws_linear_idx % num_group, (float)ws_local[0], hex_val);
//    }
// *************************************
  }
  red_buf0[0] = fp_thread_C_local[0];
  #pragma unroll
  ...
  if (threadIdx.x == 0){
    dtype_transform[out_idx] = ((red_buf0[0])/(s[0]));
  }
}