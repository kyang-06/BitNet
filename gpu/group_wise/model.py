...
class FeedForward(nn.Module):
    def __init__(
        ...
        if use_kernel:
            self.w13 = BitLinearKernel(
                dim,
                2 * hidden_dim,
                bias=False,
                ws_num=2
            )
            self.w2 = BitLinearKernel(
                hidden_dim,
                dim,
                bias=False,
                ws_num=1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x13 = self.w13(x)
        # ref = torch.mm(x, self.w13.weight.T)    # only accessible in BitLinear (prefilling)
        x1, x3 = x13.chunk(2, -1)
