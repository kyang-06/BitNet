...
extern "C" void bitlinear_int8xint2(int8_t* input0, int8_t* input1, __nv_bfloat16* output0, __nv_bfloat16* s, __nv_bfloat16* ws, int M, int N, int K, cudaStream_t stream){
    ...
    // Qwen3 (ws_num=1)
    else if (M == 1 && N == 4096 && K == 2560 ){
        ladder_int8xint2_kernel<1, 4096, 2560, 1, 8, 16><<<dim3(256, 1, 1), dim3(8, 16, 1), 0, stream>>>(input0, input1, output0, s, ws);
    }
    else if (M == 1 && N == 1024 && K == 2560 ){
        ladder_int8xint2_kernel<1, 1024, 2560, 1, 8, 16><<<dim3(64, 1, 1), dim3(8, 16, 1), 0, stream>>>(input0, input1, output0, s, ws);
    }
    else if (M == 1 && N == 6144 && K == 2560 ){
        ladder_int8xint2_kernel<1, 6144, 2560, 2, 8, 16><<<dim3(384, 1, 1), dim3(8, 16, 1), 0, stream>>>(input0, input1, output0, s, ws);
    }
        ...
        ladder_int8xint2_kernel<1, 19456, 2560, 2, 8, 16><<<dim3(1216, 1, 1), dim3(8, 16, 1), 0, stream>>>(input0, input1, output0, s, ws);
    }
